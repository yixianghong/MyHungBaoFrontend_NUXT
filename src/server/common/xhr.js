...
const xhr = async (path, method, headers, params = null, timeout = 40000, otherConfig = null) => {
    ...
    try {
        ...
        if (response.status >= 400) {
            // throw new XhrError(`Error ${response.status}, ${response.statusText}`, response.status)'
        ...
        let err = null
        if (error.response) {
            err = new Error(error.response.statusText, error.response.status)
        } else {
            err = error
        }
        const errInfo = { err, path, config }
        console.log('CALL API OCCUR ERROR!!', errInfo)
        return err
    }
}
